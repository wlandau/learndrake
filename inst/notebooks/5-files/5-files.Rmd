...
`file_in()` tells `make()` to fingerprint and track `customer_churn.csv`. As we saw in the `3-changes` exercises, `drake` updates the downstream targets when the data file changes.
