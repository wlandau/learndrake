...
5. Read this notebook and run the code chunks along the way. Look for comments labeled "YOUR TURN". These are places where you will need to write code.
...
To get the targets from the cache, use safe `drake` functions `loadd()` and `readd()`. Let's use them to check the data for potential problems.
...
How does `drake` know all this? Because it uses static code analysis: <http://adv-r.had.co.nz/Expressions.html#ast-funs>. In other words, `drake` inspects and dissects your code without actually running it. It uses the structure of your commands and functions to deduce the structure of your workflow.

hat means the order of the targets in the plan does not matter. In fact, if we reverse the recipe and the data, it has no effect.
...
The actual trained model is now in the cache.
...
Rome was not built in a day, and neither is any `drake` plan. Building up a plan is an gradual iterative process.
