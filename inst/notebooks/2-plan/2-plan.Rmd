...
Let's aggregate all the model runs into a single data frame. Define a new `runs` target with the command `bind_rows(run_relu, run_sigmoid, run_softmax)`.
...
Now, let's run the best model and tell `drake` to hold on to the trained model. Below, we use the `target()` function for extra customization because Keras models need the special `"keras"` storage format (<https://books.ropensci.org/drake/plans.html#special-data-formats-for-targets>). Below, write the command `best_model(runs, churn_recipe)`.
