...
Take a moment to open up `packages.R` and `functions.R` scripts in RStudio to look at how they are organized.

# Start the plan

Your initial plan will have code to
...
And the code to clean up the data is:
...
plan <- drake_plan(
  churn_data = read_csv(
    file_in("../data/customer_churn.csv"), # file_in() is important here
    col_types = cols()
  ) %>%
    initial_split(prop = 0.3),
  churn_recipe = prepare_recipe(churn_data)
...
When you created the plan, you did not actually run the commands. To run the commands and build targets, use `make()`.
...
The `.drake/` folder is a special kind of cache (<https://github.com/richfitz/storr>) and it is not designed for meddling users to explore.

```{r}
list.files(".drake/data")
```

Instead, use `drake` functions `loadd()` and `readd()` to get the targets back into memory. This is a good time to check the data for potential problems.
...
3. The functions you loaded when you ran `source("R/functions.R")`.
...
`drake` figures it out with static code analysis (<http://adv-r.had.co.nz/Expressions.html#ast-funs>). In other words, `drake` dissects the structure of your code without actually running it.

That means the order of the targets in the plan does not matter. In fact, let's reverse the recipe and the data.
...
And our targets is still up to date.

```{r}
make(plan)
```

If you *invalidate* the targets and run `make()` from scratch, `churn_recipe` runs second even though it is now first in the plan. `make()` uses dependency relationships to run the correct targets in the correct order.
...
Now that you built the data targets and checked them with `loadd()` and `readd()`, let's add two models. One has a relu activation function:
...
And another has a sigmoid activation function:
...
Your turn: add these targets and commands to the plan.
...
So `make()` skips the data and just runs the models. When you run `make()` below, ignore TensorFlow messages like "Your CPU supports instructions that this TensorFlow binary was not compiled to use".

```{r}
make(plan) # slow
```

Those models took a while to run, right? That is why `make()` tries to keep them up to date.
...
Now, check your model runs. Each should be a data frame with the accuracy and features of the model.
...
Now we know our first model runs came out well, so let's add a third model with a different activation function. Use the `act1 = "softmax"` this time.
...
With all our model runs in hand, let's aggregate all the results. Define a new `runs` target with the command `bind_rows(run_relu, run_sigmoid, run_softmax)`.
...
plan <- drake_plan(
  ...
  runs = # Bind the rows of all the runs.
)
```
...
It should contain 3 rows, each with a different model run.

```{r}
readd(runs)
```

Now, let's run the best model and tell `drake` to hold on to the trained model. Below, we use the `target()` function to customize the target. Keras models need a special storage format (<https://books.ropensci.org/drake/plans.html#special-data-formats-for-targets>). Below, write the command `best_model(runs, churn_recipe)`.
...
plan <- drake_plan(
  ...
  model = target( # We use target() to customize the target.
    , # Write the command, best_model(runs, churn_recipe).
...
Rome was not built in a day, and neither is your `drake` plan. Building up a plan is an iterative, gradual process. We add targets little by little and check them often.

1. Add targets to the plan.
...
Same story if you change a dataset you declared with `file_in()`.
