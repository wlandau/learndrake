...
Above, `caching = "worker"` tells `drake` to save the models in the workers instead of the master process. This is important for Keras because a model is no longer valid if you simply send it to another R process. To ensure validity across sessions, you either have to return a [serialized model](https://tensorflow.rstudio.com/keras/reference/serialize_model.html) from the `train_model()` function or [save the model as an HDF5 file](https://tensorflow.rstudio.com/keras/reference/save_model_hdf5.html).
