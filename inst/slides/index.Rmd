...
But before we even begin to use `drake`, we need to adapt our style of writing code. This project is too big for the classic "numbered script" paradigm that so many of us use. Whenever I try it, it explodes. And I'm sure I'm not the only one.
...
But more importantly, we want to be able to step back and think about what we're doing. We need modularity, and we need structure around our ideas. We want to somewhow write the flow of our project in a high-level shorthand that makes everything easier to read.
...
The way we do that is with functions. A function is a reusable command that accepts one or more inputs and returns a single output. You define it once, you give it a name, and you call it whenever you need it on whatever data that fits.
...
Functions break down complicated ideas into manageable pieces. Instead of confronting the whole thing at once, you deal with pieces that are smaller, cleaner, less intimidating, and that mean something clear and useful.
...
This may seem like extra work, but the activity of writing code is always a challenge. It always going to take focus, and it's always going to take effort. For **every single one** of us. And the more we accept that and lean into it, the better off we are in the long run, and the more things start to make sense. 
...
And we can take the functions we define and call them from other functions, including a function that runs everything at the top level and controls the overall flow of the analysis in our own high-level shorthand that makes everything easier to read.
...
Also, I said all that stuff about functions because of what I learned from recent one-on-one conversations with Ellis Hughes, Pat Schratz, Stas Kolenikov, and Michael Lawrence. They helped me see workflows how other people were seeing them. When I was in school, I took most of my computer science before most of my statistics, so I totally took functions for granted, and that made it super hard to explain `drake` in the early days. But now, thanks to people like them, I think I'm getting better.
