...
```{r largermap, exercise = TRUE, echo = FALSE, exercise.lines = 16, paged.print = FALSE}
drake_plan(
  churn_data = split_data(file_in("data/customer_churn.csv")),
  churn_recipe = prepare_recipe(churn_data), 
  run = target(
    ...
    transform = map(
      ...
      # Your turn: set units equal to c(16, 17, 18, 19, 20, 21, 22, 23) below.
      units = # ...
    )
  )
)
...
In the plan, use `!!(16:32)` instead of just `16:32`. That way, `drake_plan()` evaluates the expression `16:32` and uses the actual values. The `!!` operator ("bang-bang") comes from [tidy evaluation](https://tidyeval.tidyverse.org/).
...
drake_plan(
  ...
  run = target(
    ...
    transform = map(
      ...
      # Write !!(16:32) instead of 16:32 below.
      units = 16:23
    )
  )
)
...
Using `!!`, we can even define grouping variables outside the plan. Try it out!
...
drake_plan(
  ...
  run = target(
    test_model(act1 = act, units1 = units, churn_data, churn_recipe),
    transform = map(
      act = , # Write !!act_values to the left of the comma.
      units = # Write !!unit_values
    )
  )
)
```
...
# Write function_syms <- rlang::syms(function_names) below.
...
drake_plan(
  ...
  run = target(
    fun_run(churn_data, churn_recipe),
    # Write fun_run = !!function_syms inside map() below.
    transform = map()
  )
)
```
...
`map()` has a `.data` argument to let you iterate over a custom grid of values. Try it out below. Note: `.data` requires `!!`.
...
drake_plan(
  ...
  run = target(
    ...
    # Write .data = !!grid inside map() below.
    transform = map()
  )
)
```
...
The names of static branching targets can get quite long. Use the custom `.id` argument of `map()` to exclude the values of `fun_run` from the names. (`.id = FALSE` produces the shortest possible names.)
...
drake_plan(
  ...
  run = target(
    ...
    transform = map(
      .data = !!grid,
      # Write .id = c(act_values, unit_values).
    )
  )
)
```
...
The `cross()` transformation defines a target for every combination of grouping variables. Try it out.
...
drake_plan(
  ...
  run = target(
    ...
    transform = map( # Replace map() with cross().
...
The `combine()` transformation splices together groups of targets from `map()` and `cross()`. Below, let's combine all the runs together with `bind_rows()`.
...
`combine()` has a `.by` argument that lets you combine multiple groups of targets using grouping variables you previously defined (e.g. `combine(runs, .by = act)` or `combine(runs, .by = c(fun_run, act))`). Below, declare a group of runs for each `fun_run` function.
...
drake_plan(
  ...
  grouped_runs = target(
    bind_rows(run),
    # Write .by = fun_run to combine all the runs for each function.
    transform = combine(run)
  )
)
```
...
drake_plan(
  ...
  runs = target(
    ...
    # Write map(raw_runs, .id = fun_run)
    # to post-process each group of raw runs.
    transform = # ...
  ),
  runs_plot = target(
    plot_runs(runs, fun_run),
    # Write map(runs, fun_run, .id = fun_run) to declare a runs_plot target
    # for each group of runs and its original function.
    transform = # ...
  )
)
```
