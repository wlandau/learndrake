...
Using `!!`, we can even define grouping variables outside the plan.
...
The names of static branching targets can get quite long. The custom `.id` argument of `map()` to exclude the values of `fun_run` from the names, e.g. `.id = act_values` or `.id = c(act_values, unit_values)`. (Set `.id = FALSE` for even shorter names.)
...
The `combine()` transformation splices together groups of targets from `map()` and `cross()`. Let's combine all the runs together with `bind_rows()`.
...
Create the above plan. Declare a group of runs for each `fun_run` function.
...
Create the above plan. Use `.id = fun_run` to shorten the names of the downstream targets.
