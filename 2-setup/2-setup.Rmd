...
Now that you worked through `1-churn.Rmd`, you have the building blocks of our data analysis project. Now, let's put the pieces together in a `drake`-powered reproducible workflow.
...
But for `drake`-powered automation and reproducibility, we use a special data frame called a [`drake` plan](https://ropenscilabs.github.io/drake-manual/plans.html). **As a first exercise, take the commands from the code chunk above and fill them into the `drake` plan below.** The "targets" (`data`, `rec`, etc.) can be in any order.
...
Call `make(plan)` to actually run the workflow. `make()` builds the targets from left to right in the dependency graph. The output is stored in a hidden `.drake/` cache.

```{r}
make(plan)
```

Try `make(plan)` again. What happens?
...
The next `make(plan)` runs everything from scratch.
