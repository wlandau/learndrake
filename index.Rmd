...
Not only that, we don't have shorthand names for the steps we're doing. We don't have the structure to organize the ideas, so it's hard to take a step back and think about what we're doing.
...
A function is just a reusable command that accepts one or more inputs and returns a single output. You define it once, and you call it whenever you need it on whatever data you have.
...
But functions do so much more than that. They're not just for code you want to repeat and reuse. They're for code you want to **understand**. Functions break down complicated ideas into manageable pieces. Instead of a monolithic mess, you have components that are smaller, cleaner, and less intimidating.

Functions are like paragraphs of a data analysis. When you write functions, it's the same mental experience. You're crystallizing a stream of consciousness into clear, discrete, explainable points you want to make.
...
You're also being true to R. You're using the language as it's **designed** to be used. The mindset of "everything that exists is an object, everything that happens is a function call" is one of the truest descriptions of a function-oriented language like R. It's what R **intends** to do, and you're going with the flow.

And yes, functions are practical too because you can use them repeatedly, maybe take them out of context and test them. But that's tertiary at best. Most of the value is conceptual.
...
So let's use functions in the deep learning example. We can still have scripts, but they are better organized and no longer imperative. We separate out the packages, the functions, and the plan (which I will get to later).
...
Once we define a bunch of other functions, we can group them into higher and higher-level functions until we have a single flagship function at the very top that runs everything. This gives us a lot of control over the flow of the analysis.
...
But we can still do better. We can avoid rerunning everything indiscriminately. We can avoid micromanaging output files. And that's why we use drake.
...
Instead of a `run_everything()` function, we create a drake plan. This plan defines a bunch of skippable steps. Each step has a command, and each command returns a value called a **target**. We give these targets names on the left, and we list them in any order we want. So we can even define the data at the bottom here.
...
But as Gabe Becker likes to say, it all comes down to trust. Reproducibility is about trust. And if you're dealing with long runtimes, this trust is difficult to earn. drake can help you earn and validate it.
...
`drake` also tracks history and provenance. You can see all the versions of old targets you built in the past, when you built them, how long they took, and even which function arguments you used in your commands. Over here on the right, you can see the names of activation functions you used because "act1" was the name of a function argument clearly visible in the plan.
...
And *outside* your session, you'll find a bunch of materials online. I highly recommend the workshop. Just go to the GitHub page and click the "launch binder" badge. That'll take you to an RStudio Server instance on the cloud, and you can get started away. Karthik's holepunch package made it so smooth to set all this up. There are notebooks and apps to guide you through the whole journey of developing this deep learning workflow and diving into the toughest parts of drake.
